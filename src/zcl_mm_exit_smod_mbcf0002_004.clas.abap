class ZCL_MM_EXIT_SMOD_MBCF0002_004 definition
  public
  final
  create public .
...
CLASS ZCL_MM_EXIT_SMOD_MBCF0002_004 IMPLEMENTATION.
...
*------------------------------------------------------------------------------------------------------------------------------
      DATA(rr_vld_lgort) = zcl_sd_exc_vld_cntrl=>get_surec_val( VALUE #( typ = zcl_sd_exc_vld_cntrl=>cv_vld
                                                                   var = 'LGORT'
                                                                   val = REF #( iv_mseg-lgort ) "Kaynak depolar ara depo olmak üezere kontrol çalışacaktır.
                                                                   surec = 'ARA_DEPOLAR_STOK_VIRMAN' ) ) .
...
*------------------------------------------------------------------------------------------------------------------------------
      ...
      DATA(rr_vld_umlgo) = zcl_sd_exc_vld_cntrl=>get_surec_val( VALUE #( typ = zcl_sd_exc_vld_cntrl=>cv_vld
                                                                   var = 'LGORT'
                                                                   val = REF #( iv_mseg-umlgo ) "Hedef depo 1205 ihracat deposu olmak üzere kontrol çalışacaktır.
                                                                   surec = 'ARA_DEPOLAR_STOK_VIRMAN' ) ) .
      ASSIGN rr_vld_umlgo->* TO FIELD-SYMBOL(<lv_vld_umlgo>).

      CHECK:<lv_vld_umlgo> EQ abap_true. "Geçerli depolar için aşağıdaki kontrol çalışacaktır.
*------------------------------------------------------------------------------------------------------------------------------
      ...
      lo_msg->add( id_msgty = 'E'
                   id_msgid = 'ZMM'
                   id_msgno = '039'
                   id_msgv1 = 'ARA_DEPOLAR_STOK_VIRMAN'
                   id_msgv2 = iv_mseg-lgort
                   id_msgv3 = iv_mseg-umlgo ).
