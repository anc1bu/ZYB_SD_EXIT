*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZYB_SD_V_C007
*   generation date: 06.04.2020 at 19:17:58
*   view maintenance generator version: #001407#
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZYB_SD_V_C007       .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZYB_SD_V_C007'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZYB_SD_V_C007_EXTRACT
TOTAL = ZYB_SD_V_C007_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZYB_SD_V_C007'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZYB_SD_V_C007-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZYB_SD_V_C007_EXTRACT
TOTAL = ZYB_SD_V_C007_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZYB_SD_V_C007-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZYB_SD_V_C007'
...
TOTAL = ZYB_SD_V_C007_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZYB_SD_V_C007-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZYB_SD_V_C007-UPD_FLAG NE SPACE.
